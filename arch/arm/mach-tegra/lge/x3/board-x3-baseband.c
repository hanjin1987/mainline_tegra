/*
 * arch/arm/mach-tegra/lge/x3/board-x3-baseband.c
 *
...
#endif
static void x3_usb_hsic_phy_power(void)
{
	printk("%s\n", __func__);
...
static void x3_usb_hsic_post_phy_off(void)
{
	printk("%s\n", __func__);
#ifdef CONFIG_TEGRA_BB_XMM_POWER
	baseband_xmm_set_power_status(BBXMM_PS_L2);
...
static struct tegra_usb_phy_platform_ops hsic_imc_plat_ops = {
	...
	.port_power = x3_usb_hsic_phy_power, // How to handle port_power api?  // ebs_wondering
	.post_resume = x3_usb_hsic_post_resume, /* NEW */
	.post_phy_off = x3_usb_hsic_post_phy_off,

	//.post_phy_on = x3_usb_hsic_phy_power, // Nv-1030815_resume_fromLP0.patch
